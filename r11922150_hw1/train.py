...
def select_feat(train_data, valid_data, test_data, select_all=True):
    ...
    if select_all:
        ...
        feat_idx = [35, 36, 37, 44, 47, 48, 52, 53, 54, 55, 62, 65, 66, 70, 71, 72, 73, 83, 84]
...
def trainer(train_loader, valid_loader, model, config, device):
    ...
    for epoch in range(n_epochs):
        ...
        if mean_valid_loss < best_loss:
            best_loss, train_loss = mean_valid_loss, mean_train_loss
...
config = {
    ...
    'batch_size': 64, 
    'learning_rate': 1e-5,              
    'early_stop': 10000,    # If model has not improved for this many consecutive epochs, stop training.     
